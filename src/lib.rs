...
//!
//! async fn request_cert() -> Result<(), Error> {
//!
...
//! // that it's there.
//! let acc = dir.register_account(vec!["mailto:foo@bar.com".to_string()]).await?;
//!
...
//!     // update_my_web_server(&path, &proof).await;
//!
...
//! let ord_cert =
//!     ord_csr.finalize_pkey(pkey_pri, 5000).await?;
//!
